...
1. **Antigravity CLI.** If `agy` is not on `$PATH`, install it. Ensure the agent has appropriate tools to execute bash commands.
...
4. **API Access** — The video generation requires Vertex AI / Google Cloud API access via the python script. Ensure the host system has credentials loaded (e.g. `gcloud auth application-default login`).
...
6. **Budget — engines shown by cost, decided before anything renders.** Present the
   ...
   - **Calibrate costs, don't guess.** Run ONE still and ONE video first. Video generation can take a few minutes via the Python wrapper. Verify the output matches expectations.
...
- Run all N concurrently, detached. Command per scene:
  `agy --print "Use the image generation tool to generate: $(cat scene_i.txt). Save it as scene_i.png. Do not do anything else." > scene_i.log 2>&1`
...
- Params (omni): passed via the Python wrapper script: `--duration 8`.
- Run concurrently, detached. Re-roll individual failures. Keep the raw 1080p sources — you need their frames next.
